...
st.set_page_config(page_title="Dashboard Analisi Consegne", layout="wide")
...
# MODIFICATA: Ora restituisce anche il tempo e il risultato completo per la mappa
def calcola_percorso_ottimizzato(_gmaps_client, indirizzi_waypoint, origine, destinazione):
    if not indirizzi_waypoint:
        return 0, 0, [], None # Restituisce anche None per il risultato
    try:
...
# NUOVO: Funzione per estrarre le coordinate per la mappa
def estrai_coordinate_per_mappa(directions_result):
    ...
    # Aggiungi ogni tappa (la fine di ogni "leg")
...
file_excel = st.file_uploader("Carica il tuo foglio Excel", type=['xlsx', 'xls'])
...
if file_excel is not None:
    ...
    if df is not None:
        ...
        if not all(col in df.columns for col in colonne_richieste):
            st.error(f"Il file Excel deve contenere le colonne: {', '.join(colonne_richieste)}. Rinominale se necessario.")
        else:
            vettori_disponibili = sorted(df['COD-VETTORE'].dropna().unique().tolist())
            vettore_selezionato = st.selectbox("Seleziona un vettore da analizzare:", options=vettori_disponibili)
                ...
                if st.button("🚀 Calcola Percorso Ottimizzato"):
                    ...
                    with col2:
                        ...
                        if distanza_km > 0:
                            ...
                            # NUOVO: Mostra la metrica del tempo
                            ...
                            # NUOVO: Estrai le coordinate e mostra la mappa
